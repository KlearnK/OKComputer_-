...
export const useTeam = () => {
  ...
  // 自动为年度目标创建月度和周目标（已废弃，但保留以备将来使用）
  // @ts-ignore - 此函数可能在未来版本中使用
  ...
  const updateAnnualGoalActual = useCallback((id: string, actualBreakdown: Partial<TeamMemberAnnualGoal['breakdownGoals']>, actualExecution: Partial<TeamMemberAnnualGoal['executionGoals']>) => {
    const updated = annualGoals.map(g => 
      g.id === id ? { 
        ...g, 
