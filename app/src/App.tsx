...
// ==================== 实际内容组件 ====================
function AppContent() {
  const { teamCode, isLoading: isTeamLoading } = useTeam();
  ...
  } = useTeamCloudBase(teamCode); // 传入 teamCode
  ...
  // 加载中状态（团队初始化 + 数据加载）
  if (isTeamLoading || isDataLoading) {
    return (
  ...
  // 处理编辑目标
const handleEditGoal = (goal: TeamMemberAnnualGoal | AnnualGoal) => {
  setEditingGoal(goal as TeamMemberAnnualGoal);
  setSelectedMemberIdForGoal((goal as any).memberId || '');
